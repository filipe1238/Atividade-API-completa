...
routerAPIv2.get("/produtos", checkToken, function (req, res) {
  knex
    ...
    .catch((err) =>
      res.json({ message: `Erro ao recuperar produtos: ${err.message}` })
    );
});
...
// Cria um manipulador da rota padrão
routerAPIv2.post("/produtos", checkToken, function (req, res) {
  knex("produtos")
    .insert(req.body, ["id"])
    .then((produtos) => {
...
// remover produto
routerAPIv2.delete("/produtos/:id", checkToken, function (req, res) {
  let id = req.params.id;
  knex("produtos")
    .where({ id })
    .del()
    .then(() => res.json({ message: `Produto removido com sucesso.` }))
    .catch((err) =>
      res.json({ message: `Erro ao remover produto: ${err.message}` })
    );
});
